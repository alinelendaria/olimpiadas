      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-start justify-between mb-8 flex-wrap gap-4">
          ...
          {profile ? (
            <div className="bg-amber-50 border border-amber-100 rounded-xl px-4 py-3 text-right">
              ...
              <p className="text-xl font-extrabold text-amber-700">🪙 {profile.saldo.toLocaleString()}</p>
            </div>
          ) : (
        ...
        {profile && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
            {[
              { label: 'Total apostado', value: `🪙 ${profile.total_apostado.toLocaleString()}` },
              { label: 'Ganhos',         value: `🪙 ${profile.total_ganho.toLocaleString()}` },
              { label: 'Win rate',       value: (profile.apostas_ganhas + profile.apostas_perdidas) > 0
                  ...
                  : '—' },
              { label: 'Maior vitória',  value: `🪙 ${profile.maior_vitoria.toLocaleString()}` },
            ].map(s => (
                  ...
                  <div key={bet.id} className="bg-white border border-gray-100 rounded-xl px-4 py-3 shadow-card flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <div className="text-right">
                        <p className="text-sm font-bold text-gray-900">🪙 {bet.valor}</p>
                        {bet.status === 'ganha' && (
